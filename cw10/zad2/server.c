...
void accept_message(int socket_fd)
{
    ...
    switch(pck.msg_type)
    {
        case CLIENT_REGISTER:
            ...
            send_packet(socket_fd, client_address, addr_size,
                SERVER_ACCEPT, "Calculation server welcomes You\nWait for instructions", -1, client_counter);
        ...
        case CLIENT_PONG:
            ...
            break;
        case CLIENT_ANSW:
            printf("operation "ANSI_GREEN"#%i"ANSI_RESET" => %s\n"ANSI_GREEN"#%i: "ANSI_RESET, \
                pck.c_count, buff, counter);
            fflush(stdout);
            break;
        case CLIENT_ERR:
            printf(ANSI_RED"%s"ANSI_GREEN"\n#%i: "ANSI_RESET, buff, counter);
            fflush(stdout);
            break;
        case CLIENT_UNREG:
            pthread_mutex_lock(&list_mutex);
...
void* terminal(void *arg)
{
    ...
    size_t n = 0;
    int count, sock;
    printf(ANSI_GREEN"#0: "ANSI_RESET);
    for (;;)
    {
        ...
        else printf(ANSI_RED"No clients connected yet" \
